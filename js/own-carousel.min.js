...
this.querySelector(".control__next").addEventListener("click",function(){a.moveSlide(1)}));var r=!1;if(l){var t=currentPos=0;l=function(c){c.preventDefault();a.carousel.style.transition="none";"touchstart"===c.type?(currentPos=c.touches[0].clientX,document.addEventListener("touchmove",m),document.addEventListener("touchend",n)):(a.carouselOuter.style.cursor="grab",currentPos=c.clientX,document.addEventListener("mousemove",m),document.addEventListener("mouseup",n));t=currentPos;h&&(clearInterval(k),
clearTimeout(p));r=!0};var m=function(c){var f=parseFloat(a.carousel.style.transform.slice(12)),q=-f/a.step;c="touchmove"===c.type?c.touches[0].clientX:c.clientX;var u=c-currentPos;g?0>=q?(a.index=a.numberOfItem,a.translateSlide()):q>=a.numberOfItem+a.itemPerRow?(a.index=a.itemPerRow,a.translateSlide()):a.carousel.style.transform="translate3d("+(f+u)+"px,0,0)":a.carousel.style.transform=0>q||q>a.numberOfItem-a.itemPerRow?"translate3d("+(f+u/5)+"px,0,0)":"translate3d("+(f+u)+"px,0,0)";currentPos=c};
this.checkIndex=function(c){for(var f=c;f>=a.step;)f-=a.step;return 50<f&&0<=t-currentPos||50>a.step-f&&0>=t-currentPos?Math.ceil(c/a.step):Math.floor(c/a.step)};var n=function(c){"touchend"===c.type?(document.removeEventListener("touchmove",m),document.removeEventListener("touchend",n)):(document.removeEventListener("mousemove",m),document.removeEventListener("mouseup",n),a.carouselOuter.style.cursor="auto");c=parseFloat(a.carousel.style.transform.slice(12));a.index=a.checkIndex(-c);g||(a.index>
a.numberOfItem-a.itemPerRow&&(a.index=a.numberOfItem-a.itemPerRow),0>a.index&&(a.index=0));a.carousel.style.transition="all 0.25s";a.translateSlide();h&&(p=setTimeout(function(){k=setInterval(a.moveSlide,h,1)},2E3));r=!1};this.carouselOuter.addEventListener("mousedown",l);this.carouselOuter.addEventListener("touchstart",l)}w&&this.carouselOuter.addEventListener("wheel",function(c){c.preventDefault();0<c.deltaY?a.moveSlide(-1):a.moveSlide(1)});var k;if(h){var p=setTimeout(function(){k=setInterval(a.moveSlide,
h,1)},3E3);x&&(this.carouselOuter.addEventListener("mouseenter",function(){clearTimeout(p);clearInterval(k)}),this.carouselOuter.addEventListener("mouseleave",function(){r||(p=setTimeout(function(){k=setInterval(a.moveSlide,h,1)},2E3))}))}};function debounce(d,a){var e=null;return function(b){clearTimeout(e);e=null;e=setTimeout(function(){d.call(this,b)},a)}}
